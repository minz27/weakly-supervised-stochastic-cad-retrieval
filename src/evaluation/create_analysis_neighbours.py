...
category_list = ['02818832','04256520','03001627','02933112','04379243','02871439']
...
def loss_to_evaluate(normal_1:torch.tensor, normal_2:torch.tensor, 
    ...
    # hist_distance = angular_loss(abs(normal_1), abs(normal_2))
...
def find_k_nearest_neighbours(masked_imgs, masked_normals, views, shape_normals, shape_batch, alpha:float=1., beta:float=1., gamma:float=0., n_views:int=3)->pd.DataFrame:
    ...
    #     scan_normal = abs(masked_normals[i].permute(1,2,0).detach().cpu().numpy())
    ...
    #     shape_normal = abs(shape_normals[i].permute(1,2,0).detach().cpu().numpy())
    ...
    # with open('src/runs/cached_distances_new_v7.pkl', 'wb') as file:
    ...
    with open('src/runs/cached_distances_new_v7.pkl', 'rb') as file:
...
if __name__=='__main__':
    ...
    results_file = f'results_top_{K}_v7.csv'
